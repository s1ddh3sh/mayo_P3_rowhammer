...
// seed_sk $←- B^(sk_seed bytes)
  ...
  expand_P1_P2(p, P, seed_pk);
  memcpy(P2_orig, P2, P2_LIMBS_MAX);

  // compute P3 (modifies P2 in the process)
  compute_P3(p, P1, P2, O, P3);
  int e = memcmp(P2, P2_orig, P2_LIMBS_MAX);
  printf(" Is equal %d \n", e);
  ...
  pack_m_vecs(P3_upper, cpk + param_pk_seed_bytes, param_P3_limbs / m_vec_limbs,
              param_m);
//   FILE *fp = fopen("../pk_faulty.txt", "w");

//   for (size_t i = 0; i < PARAM_cpk_bytes(p); ++i) {
//     fprintf(fp, "%02x", cpk[i]);
//   }

//   fprintf(fp, "\n");
//   fclose(fp);

  // FILE *fp1 = fopen("seed_sk.bin", "wb");
  // fwrite(seed_sk,
